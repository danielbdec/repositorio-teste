...
export function MergedEventCard({ event }: MergedEventCardProps) {
    ...
    return (
        <div ref={setNodeRef} style={style} {...listeners} {...attributes} className="touch-none mb-3 relative group">
            ...
            <div className="absolute top-[-4px] left-[4px] right-[4px] h-full bg-muted/40 rounded-lg border border-border -z-10" />
            <div className="absolute top-[-8px] left-[8px] right-[8px] h-full bg-muted/20 rounded-lg border border-border -z-20" />

            {/* Card Principal */}
            <div
                className={cn(
                    "relative p-3 rounded-lg backdrop-blur-md transition-all cursor-grab active:cursor-grabbing",
                    "bg-card/80 border border-agri-gold-500/30",
            ...
            >
                ...
                <div className="flex items-center justify-between mb-3 pb-2 border-b border-border">
                    <div className="flex items-center gap-2">
                        ...
                        <div>
                            <h4 className="font-bold text-sm text-foreground leading-tight">Evento Otimizado</h4>
                            <span className="text-[10px] text-muted-foreground font-medium">{event.protocols.length} Operações Combinadas</span>
                        </div>
                    </div>
                    ...
                    {event.protocols.map((p, idx) => (
                        <div key={p.id} className="flex items-center justify-between text-xs group/item">
                            <div className="flex items-center gap-2 overflow-hidden">
                                <span className="w-1.5 h-1.5 rounded-full bg-muted-foreground group-hover/item:bg-agri-gold-500 transition-colors" />
                ...
                {/* Footer Totais */}
                <div className="flex items-center justify-between pt-2 border-t border-border bg-muted/20 -mx-3 -mb-3 px-3 py-2 rounded-b-lg">
                    <div className="flex flex-col">
                        <span className="text-[9px] text-muted-foreground uppercase tracking-widest font-bold">Custo Total</span>
