    ...
    return (
        <div className="relative border border-border dark:border-white/10 rounded-xl overflow-hidden bg-white dark:bg-card/40 dark:bg-gradient-to-br dark:from-white/5 dark:to-white/0 backdrop-blur-none dark:backdrop-blur-md shadow-sm dark:shadow-lg mb-4">
            <div className="relative group">
                ...
                <div
                    className="absolute inset-0 dark:bg-gradient-to-r dark:from-agri-green-500/0 dark:via-agri-green-500/5 dark:to-agri-green-500/0 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"
                />

                <div className="flex items-stretch min-h-[50px] border-b border-border dark:border-white/5">
                    {/* Header da Swimlane (Icon + Title) */}
                    <div
                        className="w-48 flex-shrink-0 flex items-center gap-3 p-3 cursor-pointer hover:bg-secondary dark:hover:bg-white/5 transition-colors rounded-l-lg border-r border-border dark:border-white/5 bg-secondary/50 dark:bg-slate-950/20"
                        onClick={() => setIsExpanded(!isExpanded)}
                    >
                        {isExpanded ? <ChevronDown size={16} className="text-muted-foreground dark:text-slate-500" /> : <ChevronRight size={16} className="text-muted-foreground dark:text-slate-500" />}
                        <div className="p-1.5 rounded-lg bg-agri-green-50 dark:bg-agri-green-950/50 border border-agri-green-200 dark:border-agri-green-900/50 shadow-inner group-hover:border-agri-gold-500/30 transition-colors">
                            <Icon className="w-4 h-4 text-agri-green-700 dark:text-agri-green-500 group-hover:text-agri-gold-500 transition-colors" />
                        </div>
                        <span className="font-bold text-xs text-foreground dark:text-slate-300 tracking-tight group-hover:text-foreground dark:group-hover:text-white transition-colors uppercase">
                    ...
                    {!isExpanded && (
                        <div className="flex items-center gap-6 text-xs text-muted-foreground animate-in fade-in slide-in-from-left-2 pl-6 py-3">
                            <div className="flex flex-col">
                                <span className="text-[10px] uppercase text-muted-foreground dark:text-slate-500 font-bold">Eventos</span>
                                <span className="font-mono text-foreground dark:text-slate-300">{totalEvents}</span>
                            </div>
                            <div className="w-px h-6 bg-border dark:bg-white/5"></div>
                            <div className="flex flex-col">
                                <span className="text-[10px] uppercase text-muted-foreground dark:text-slate-500 font-bold">Acumulado/ha</span>
                                <span className="font-mono text-agri-gold-600 dark:text-agri-gold-400 font-bold">{formatCurrency(aggregatedCostPerHa)}</span>
                            </div>

                            {/* Badge de Alerta se custo alto (Mock) */}
                            {aggregatedCostPerHa > 500 && (
                                <span className="px-2 py-0.5 rounded bg-red-100 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 text-red-600 dark:text-red-400 text-[10px] font-bold">
                                    ALTO CUSTO
                                </span>
                            )}
                        </div>
